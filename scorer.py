...
def score_importance(
    ...
    system_prompt = (
        ...
        "In a typical scan of 40-50 articles only 2-3 should score 9-10. "
        "Most articles should score 5-7. "
        "Only score 9-10 for genuinely breaking news like election announcements, "
        "cabinet collapses, or major political crises. "
        "A routine cabinet formation or appointment is 6-7, not 9-10. "
        "A leadership change is 7-8. "
        "Only active electoral developments or political crises score 9-10. "
        "Scoring guide: "
        ...
        f"affects_client_region must be true if this article is about events in {district} or {state}. "
        f"This includes: (a) articles that explicitly name {district} or {state}; "
        f"(b) articles where {state} is clearly implied by context — for example, coverage by a "
        f"{state} regional outlet discussing the state CM, state assembly, state government, or "
        f"state-level party affairs without spelling out '{state}' every sentence; "
        f"(c) articles in a regional language (e.g. Kannada for Karnataka) whose topic is "
        f"state politics even if the state name does not appear in the translated snippet. "
